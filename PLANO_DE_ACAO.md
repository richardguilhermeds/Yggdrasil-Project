...
- [ ] **P24 — Duplicação de helpers entre tree e model** — `model/segmenter.py:194` (e correspondentes na árvore).
...
- [ ] **P25 — `_classifica_psi` da árvore não trata None/NaN** — `tree/segmenter.py:95`.
  Classifica PSI inválido como "instável". → Guard `if not np.isfinite(psi): return '—'` (alinha com o model).
- [ ] **P26 — `_classifica_iv`: default de `task_type` diverge** — `model/segmenter.py:214` vs `tree/segmenter.py:113`.
...
- [ ] **P27 — `backtest` (tol absoluta) vs `plot_backtest` (banda relativa)** — `model/segmenter.py:4550`.
  Tabela e gráfico podem discordar sobre "fora da banda". → Unificar a definição de tolerância.
- [ ] **P28 — `(P)` SHAP beeswarm/bar usam `plt.figure`/`gcf` sem `close`** — `model/segmenter.py:3877`.
  Possível display duplicado/vazamento de figuras no inline. → `plt.close(fig)` antes do return.
- [ ] **P29 — Trials degenerados (métrica NaN) viram COMPLETE e `fit_best` reajusta** — `model/segmenter.py:2404`.
  Sentinela `-1e9` deixa o estudo "concluído" mesmo vazio. → Tratar `best_value<=-1e8` como degenerado (não reajustar, avisar).
- [ ] **P30 — Forest plot do bootstrap clampa eixo em [0,1]** — `tree/ui.py:3565`.
  Quebra alvos de regressão (LGD) fora do intervalo. → Só clampar quando `_is_clf`.
- [ ] **P31 — `plot_leaf_boxplots` fixa `ylim(0,1)`/`Normalize(0,1)`** — `tree/segmenter.py:2272`.
  Recorta caixas de LGD fora de [0,1]. → Ajustar aos dados observados.
- [ ] **P32 — Tokens de tema de PSI divergentes** — `tree/ui.py:1871`.
  Árvore usa `--risk-*`; cartões usam `--ok/warn/bad-tx`. → Padronizar nos tokens semânticos.
- [ ] **P33 — Gauge de PSI não trata NaN** — `tree/ui.py:3159`.
  `nan%` quebra a barra e vira classe "red". → Filtrar NaN / tratar como "—".
- [ ] **P34 — Contraste WCAG no heatmap categórico (fallback)** — `tree/ui.py:476`.
  Texto branco sobre fundo claro (~2:1). → Escolher preto/branco pela luminância real.
- [ ] **P35 — Escala de cor da árvore usa `vmax` de todas as amostras (rótulo diz DES)** — `tree/segmenter.py:1504`.
  Cor/rótulo incoerentes quando OOT puxa a média. → Restringir `vmax` à máscara de referência.
- [ ] **P36 — Limite de params log-scale não validado (>0) no tuning** — `model/ui.py:2301`.
  `low<=0` com `log=True` faz todos os trials falharem. → Piso positivo + aviso.
- [ ] **P37 — "Escolha ótima" reusa backward ignorando a amostra atual** — `model/ui.py:1861`.
  Usa métricas da amostra antiga. → Incluir amostra/algoritmo na condição de reuso.
- [ ] **P38 — `(P)` `_on_backelim` não clampa `min_features`** — `model/ui.py:2940`.
  Slider com max obsoleto pode rodar poucos/nenhum passo. → Clampar como `_on_feat_optimal`.
- [ ] **P39 — `(P)` Trocar algo/HP após treino não marca dirty** — `model/ui.py:1096`.
  Métricas/fórmula do modelo antigo sem aviso de obsolescência. → Encadear `_mark_dirty` (ou documentar por que não).
- [ ] **P40 — p-valores de Wald mapeados por nome de exibição colidem** — `model/segmenter.py:2745`.
  Rótulos idênticos colapsam a dict → p-valor errado. → Mapear por posição/índice do termo.
- [~] **P41 — `_leaf_id_series`: chave de memo ignora `leaf_ids`** — `tree/segmenter.py:1426`. **(parcial)**
  ✅ Já corrigido o crash sob pandas 3.0 (CoW): não mais `s.values[...]=` in-place. ⏳ Falta: incluir `leaf_ids` na chave de memo (hoje inofensivo — todos os chamadores passam o conjunto completo/None).
- [ ] **P42 — `(P)` `_agg_memo` faz cópia rasa de dict/list** — `tree/segmenter.py:436`.

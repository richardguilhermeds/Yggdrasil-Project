...
.pdui-metrics { display:grid; grid-template-columns:repeat(auto-fill,minmax(92px,1fr));
  gap:6px; }
...
class PDSegmenterUI:
    ...
    def _build(self):
        ...
        # ================================================================
        col_decision = W.VBox([
            W.HTML("<div class='pdui-h'>Qual variável segmentar? (IV &amp; PSI por variável)</div>"),
            iv_legend, self.out_iv,
            W.HTML("<div class='pdui-h' style='margin-top:12px'>PD da folha analisada</div>"),
            W.HTML("<div class='pdui-legend'>Taxa de default da folha selecionada (DES), com "
                   "IC de Wilson e a PD da carteira como referência.</div>"),
            self.out_leaf_hist,
        ], layout=W.Layout(width="32%"))
        col_decision.add_class("pdui-card")
        ...
        card_tree = W.VBox([
            ...
            W.HTML("<div class='pdui-h' style='margin-top:10px'>Auto-fit</div>"),
        ...
        card_preview = W.VBox([
            W.HTML("<div class='pdui-h'>Pré-visualização da divisão</div>"),
            self.out_preview_chart,
        ])
        card_preview.add_class("pdui-card")
        ...
        card_assist = W.VBox([
            W.HTML("<div class='pdui-h'>Assistente</div>"),
            W.HTML("<div class='pdui-legend'><b>Sugerir split</b> — aponta a variável de "
            ...
            W.HTML("<div class='pdui-h' style='margin-top:12px'>Auto-fundir folhas-irmãs</div>"),
            W.HTML("<div class='pdui-legend'><b>Auto-fundir</b> — funde automaticamente os "
                   "pares de <b>folhas-irmãs indistinguíveis</b>: quando o teste de hipótese "
                   "entre elas dá <b>p &gt; alpha</b> (sem evidência de PD diferente), as duas "
            ...
            W.HTML("<div class='pdui-h' style='margin-top:12px'>Podar folhas-irmãs</div>"),
            W.HTML("<div class='pdui-legend'><b>Podar</b> — funde folhas-irmãs com "
                   "<b>repr. &lt; min repr%</b> (imateriais) ou <b>ΔPD &lt; mínimo</b> "
                   "(ex.: 0,02 = 2 p.p.). O seletor <b>Teste</b> define o p-valor da tabela de "
                   "folhas (Diagnóstico) e do auto-fundir.</div>"),
            self.sl_repr, self.sl_gap, self.dd_test,
            W.HBox([self.btn_prune]),
        ])
        card_assist.add_class("pdui-card")
        ...
        card_tree_img.add_class("pdui-card")
        ...
        tab_diag = W.VBox([card_table, card_discrim, card_metrics, card_boot, card_quality])

        # ================================================================
        # ABA ④ VALIDAR & EXPORTAR — validação regulatória · exportar régua
        # ================================================================
        valid_legend = W.HTML(
            "<div class='pdui-legend'>Validação regulatória: <b>monotonicidade</b> da PD nas "
        ...
        card_validacao = W.VBox([
            W.HTML("<div class='pdui-h'>Validação &amp; relatório (monotonicidade · calibração · backtest)</div>"),
            valid_legend,
            W.HBox([self.tx_time_col, self.btn_validate],
                   layout=W.Layout(align_items="center")),
            self.out_validate,
            W.HTML("<div class='pdui-h' style='margin-top:8px'>Relatório de validação (Markdown)</div>"),
        ...
        card_export = W.VBox([
            ...
            W.HTML("<div class='pdui-h' style='margin-top:10px'>Salvar no MLflow</div>"),
            self.tx_model, self.cb_uc, self.tx_experiment, self.tx_runname,
            W.HBox([self.btn_mlflow]),
            W.HTML("<div class='pdui-h' style='margin-top:10px'>Reconstruir folhas em tabela Spark</div>"),
            self.tx_spark_in, self.tx_spark_out,
            W.HBox([self.btn_spark_apply]),
        ])
        card_export.add_class("pdui-card")
    ...
    def _leaf_header_html(self):
        ...
        estado (aberta/fechada), nº da folha e métricas-chave — n, repr.%, PD e
        ...
        return (
            ...
            f"{badge}</div><div class='pdui-metrics'>{cell_html}</div>")
    ...
    def _refresh_leaf_hist(self):
        ...
        if sid is None or sid not in self.seg.segments:
            ...
            return
        try:
            self.out_leaf_hist.value = self._fig_html(self.seg.plot_leaf_target_hist(sid))
        except Exception as e:
    ...
    def _var_cards_html(self, s, trend):
        ...
        iv = s.get("iv")
        if iv is not None:
            rows.append(card("IV (binário)", f"{iv:.4f}", s.get("forca", "—")))
        stat_html = ""
        if s.get("tipo") == "num" and s.get("media") is not None:
            def st(k, kk):
                v = s.get(kk)
                return card(k, f"{v:.3f}" if v is not None else "—")
            stat_html = ("<div class='pdui-metrics' style='margin-top:8px'>"
            ...
            stat_html = ("<div class='pdui-metric' style='margin-top:8px;padding:8px 11px'>"
                         "<div class='k'>Categorias (share)</div>" + linhas + "</div>")
        return "<div class='pdui-metrics'>" + "".join(rows) + "</div>" + stat_html
    ...
    def _on_preview(self, _):
        ...
        with self.out_log:
            ...
            graf = ("construção (barras × PD) + histograma" if kind == "num"
                    else "construção (barras × PD)")
        ...
        partes = []
        try:
            partes.append(self._fig_html(self.seg.plot_feature_pd(

...
class SequentialLGDSegmenter:
    ...
    def _resolve_bins(self, sub, feature, splits, dtype, max_n_bins, min_bin_size,
        ...
        min_mean_diff: diferença mínima de LGD médio exigida entre bins
    ...
    def grow(self, feature, splits=None, dtype=None, max_n_bins=4,
        ...
        for sid, seg in targets.items():
            ...
            bins, modo, kind = self._resolve_bins(
                sub, feature, splits, dtype, max_n_bins, min_bin_size, max_bin_size,
                relax_max=relax_max, min_mean_diff=min_mean_diff)
            modo_usado = modo
            if not bins:
    ...
    def plot_feature_hist(self, feature: str, sid: str | None = None, splits=None,
        ...
        if kind != "num":
            ...
            return self.plot_feature_lgd(feature, sid=sid, splits=splits, dtype=dtype,
                                         ...
                                         max_bin_size=max_bin_size, min_mean_diff=min_mean_diff,
    ...
    # ==================================================================
    # FOLHAS-IRMÃS: comparação do LGD médio entre folhas de MESMO PAI, por
    #   amostra e por safra, com detecção de INVERSÃO de ordenação. Duas irmãs
    #   "invertem" quando a ordem do LGD médio observada numa amostra/safra
    #   contradiz a ordem de referência (LGD na DES) — sinal de instabilidade
    #   da segmentação (o ranking de risco não se sustenta no tempo/fora da
    #   amostra de desenvolvimento).
    # ==================================================================
    def _ordered_samples_with_lgd(self) -> list:
        """Amostras que têm LGD observado, com a referência (DES) à frente."""
        if self.sample_col is None:
            return []
        samples = list(self.df[self.sample_col].dropna().unique())
        com_lgd = [a for a in samples
                   if self.df.loc[self.df[self.sample_col] == a, self.target].notna().any()]
        ref = [self.ref_sample] if self.ref_sample in com_lgd else []
        return ref + [a for a in com_lgd if a != self.ref_sample]

    def _sibling_meta(self, parent_sid, leaves=None):
        """Metadados das folhas-irmãs de `parent_sid`:
        (ordenadas_por_LGD_DES, nota_por_sid, descrição_curta_por_sid,
        lgd_ref_por_sid). A ORDEM DE REFERÊNCIA é o LGD médio na DES (asc.)."""
        lv = self.leaves()
        nota = dict(zip(lv["segmento"], lv["nota_lgd"]))
        if leaves is None:
        ...
        lgd_ref = {}
        for sid in leaves:
            v = self._leaf_target(sid)          # LGD na referência (DES), sem NaN
            lgd_ref[sid] = float(v.mean()) if len(v) else float("nan")
        ordered = sorted(leaves, key=lambda c: (np.inf if pd.isna(lgd_ref[c])
                                                else lgd_ref[c], str(c)))
        desc = {sid: self._descrever(self.segments[sid]["conditions"][-1:])
                for sid in leaves}
        return ordered, nota, desc, lgd_ref
    ...
    def sibling_leaf_groups(self, min_leaves: int = 2) -> list:
        ...
        for pai, kids in grupos.items():
            if len(kids) < min_leaves:
                continue
            ordered, nota, _desc, _lgd = self._sibling_meta(pai, kids)
            feat = None
    ...
    def _sibling_sample_series(self, parent_sid, leaves=None):
        """Série do LGD médio por AMOSTRA para cada folha-irmã.
        Retorna (ordered, nota, desc, xs, series) com series[sid] = [médias]."""
        ordered, nota, desc, _lgd = self._sibling_meta(parent_sid, leaves)
        samples = self._ordered_samples_with_lgd()
    ...
    def _sibling_safra_series(self, parent_sid, time_col=None, sample=None,
        ...
        """Série do LGD médio por SAFRA (mês de `time_col`) para cada folha-irmã.
    ...
    def _count_inversions(ordered, values) -> tuple:
        ...
        pares comparáveis. `values` = dict sid->LGD num ponto (amostra/safra).
        Par (i<j na referência) inverte quando LGD_i > LGD_j."""
        n_inv = n_pairs = 0
    ...
    def sibling_inversion_summary(self, parent_sid, time_col=None, sample=None,
        ...
        ordem de LGD de referência (DES) com a observada em cada AMOSTRA e em
        cada SAFRA. Retorna contagens por amostra/safra e um veredito
        (verde/amarelo/vermelho)."""
        ordered, nota, desc, lgd_ref = self._sibling_meta(parent_sid)
        k = len(ordered)
        n_pairs = k * (k - 1) // 2
        ...
        if sample_inv > 0 or safra_rate > 0.25:
            ...
            status = "green"
        return {"ordered": ordered, "nota": nota, "desc": desc, "lgd_ref": lgd_ref,
    ...
    def sibling_lgd_by_sample(self, parent_sid, leaves=None) -> pd.DataFrame:
        """Tabela tidy: LGD médio de cada folha-irmã por amostra."""
        ordered, nota, desc, xs, series = self._sibling_sample_series(parent_sid, leaves)
        rows = []
        for sid in ordered:
            row = {"segmento": sid, "nota_lgd": nota.get(sid), "descricao": desc[sid]}
            for x, v in zip(xs, series[sid]):
                row[x] = round(v, 4) if not pd.isna(v) else np.nan
            rows.append(row)
        return pd.DataFrame(rows)

    def sibling_lgd_by_safra(self, parent_sid, time_col=None, sample=None,
                             leaves=None, min_n: int = 1) -> pd.DataFrame:
        """Tabela tidy: LGD médio de cada folha-irmã por safra (linhas = safra)."""
    ...
    def _sibling_colors(self, ordered):
        """Cor por folha (verde→vermelho na ordem de LGD) p/ os gráficos."""
        import matplotlib.pyplot as plt
    ...
    def plot_sibling_lgd_by_sample(self, parent_sid, leaves=None,
                                   figsize=(7.6, 4.0), dpi=150,
                                   save_path=None, ax=None):
        """Linhas do LGD médio das folhas-irmãs por amostra (DES, OOT, …). Onde
        as linhas se cruzam há INVERSÃO da ordem de risco entre as irmãs."""
        try:
            import matplotlib.pyplot as plt  # noqa: F401
        except ImportError as e:  # pragma: no cover
            raise ImportError("plot_sibling_lgd_by_sample requer matplotlib.") from e
        ordered, nota, desc, xs, series = self._sibling_sample_series(parent_sid, leaves)
        fig, ax = self._new_ax(figsize, dpi, ax)
        ...
        ax.set_ylabel("LGD médio"); ax.set_xlabel("amostra")
        ax.set_title("LGD médio das folhas-irmãs por amostra",
                     fontsize=11, fontweight="bold", color="#15324a")
        ax.grid(axis="y", alpha=0.15)
    ...
    def plot_sibling_lgd_by_safra(self, parent_sid, time_col=None, sample=None,
                                  leaves=None, min_n: int = 1,
                                  figsize=(9.6, 4.0), dpi=150,
                                  save_path=None, ax=None):
        """Linhas do LGD médio das folhas-irmãs por safra ao longo do tempo. As
        safras em que a ordem de risco inverte (vs. DES) ficam sombreadas em
        vermelho — leitura rápida de instabilidade temporal."""
        try:
            import matplotlib.pyplot as plt  # noqa: F401
        except ImportError as e:  # pragma: no cover
            raise ImportError("plot_sibling_lgd_by_safra requer matplotlib.") from e
        ordered, nota, desc, xs, series = self._sibling_safra_series(
            parent_sid, time_col, sample, leaves, min_n=min_n)
        fig, ax = self._new_ax(figsize, dpi, ax)
        if not ordered or not xs:
        ...
        ax.set_ylabel("LGD médio"); ax.set_xlabel("safra")
        sfx = f" · {sample}" if sample else " · todas as amostras"
        ax.set_title(f"LGD médio das folhas-irmãs por safra{sfx}"
    ...
    def _concentration_bins(self, sub, feature, min_leaf_repr, max_bin_repr, dtype=None):
        ...
        por LGD em DES (categórico) e fundimos os bins adjacentes pequenos —
        ...
        if kind == "num":
            ...
            # uma categoria por grupo, ordenadas por LGD em DES (contiguidade de risco)
        ...
        # bin de faltantes não funde com faixas/grupos: se < mínimo, variável inviável
        na = [{"kind": "na"}] if na_present else []
        if na and min_leaf_repr is not None and grepr(na[0]) < min_leaf_repr - 1e-9:
            return None, kind

        # fusão gulosa de vizinhos adjacentes (num e cat já estão ordenados)
        ...
        if len(core) < 2:
            return None, kind        # sobrou 1 faixa/grupo → sem split útil
        out = core + na
        if min_leaf_repr is not None and any(grepr(b) < min_leaf_repr - 1e-9 for b in out):
            return None, kind        # não deu p/ todos ≥ mínimo
        if max_bin_repr is not None and any(grepr(b) > max_bin_repr + 1e-9 for b in out):
            return None, kind        # algum bin acima do máximo → variável inviável
        return out, kind
        ...
        * ``max_bin_repr`` — nenhuma quebra pode concentrar mais que essa fração da
        ...
        for depth in range(base_depth, base_depth + max_depth):
            ...
            for sid in atuais:
                ...
                # amostras), mas o optbinning ajusta só em DES e o bin de faltantes
                # (NaN) não passa pelo min_bin_size. Para cada variável (por IV),
                # ajusta os cortes à concentração global e cresce a 1ª que respeite
                # o mín/máx por folha; senão a folha fica terminal.
                for _, row in iv.iterrows():
                    if pd.isna(row["iv"]) or row["iv"] < min_iv:
                        break                       # IV ordenado desc → nada mais serve
                    var = row["variavel"]
                    try:
                        ...
                        continue
                    if bins_fit is None:
                        continue                    # variável não respeita mín/máx global
                    if kind == "num":
                    ...
                    if not splits:
                        continue
                    # cresce exatamente os bins ajustados (modo manual = sem re-fit)
                    with contextlib.redirect_stdout(io.StringIO()):

...
def _fake_table(problem: str = "classification") -> pd.DataFrame:
    ...
    rows = [
        linha("feat_bureau_score", "bureau", sel=True, motivo="selecionada (consenso)",
              score=20, rf=0.30, corr=0.8, iv=0.55, ks=0.5, auc=0.9, gini=0.8,
              hits=11, dec="confirmada", cons=0.9, cluster=1),
        linha("feat_bureau_atraso_max", "bureau", sel=False,
              motivo="redundante c/ feat_bureau_score", score=18, rf=0.25, corr=0.78,
              iv=0.5, ks=0.48, cluster=1, rep=False, red="feat_bureau_score"),
        ...
        linha("feat_bvs_miss", "bvs", sel=False, motivo="alto missing", miss=0.82,
              cluster=3),
        linha("feat_bureau_const", "bureau", sel=False, motivo="sem variância",
        ...
        linha("feat_openfinance_vazamento", "openfinance", sel=False,
              ...
              leak=True, cluster=6),
        linha("feat_bureau_qt_negativacoes", "bureau", sel=False, motivo="Boruta rejeitada",

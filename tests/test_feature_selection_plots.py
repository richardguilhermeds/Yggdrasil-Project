...
def _fake_table(problem: str = "classification") -> pd.DataFrame:
    ...
    rows = [
        linha("feat_externo_score", "externo", sel=True, motivo="selecionada (consenso)",
              score=20, rf=0.30, corr=0.8, iv=0.55, ks=0.5, auc=0.9, gini=0.8,
              hits=11, dec="confirmada", cons=0.9, cluster=1),
        linha("feat_externo_atraso_max", "externo", sel=False,
              motivo="redundante c/ feat_externo_score", score=18, rf=0.25, corr=0.78,
              iv=0.5, ks=0.48, cluster=1, rep=False, red="feat_externo_score"),
        ...
        linha("feat_mercado_miss", "mercado", sel=False, motivo="alto missing", miss=0.82,
              cluster=3),
        linha("feat_externo_const", "externo", sel=False, motivo="sem variância",
        ...
        linha("feat_openfinance_vazamento", "openfinance", sel=False,
              ...
              leak=True, cluster=6),
        linha("feat_externo_qt_negativacoes", "externo", sel=False, motivo="Boruta rejeitada",
